...
func TestRunWikiNodeCreateClassifiesLayerLimitAsNonRetryable(t *testing.T) {
	t.Parallel()
	...
	limitErr := errs.NewAPIError(errs.SubtypeUnknown, "opaque upstream message").
		WithCode(wikiNodeCreateLayerLimitCode).
		WithRetryable().
	...
	if problem.Category != errs.CategoryAPI || problem.Code != wikiNodeCreateLayerLimitCode || problem.Retryable {
		t.Fatalf("problem = %#v, want API code %d and retryable=false", problem, wikiNodeCreateLayerLimitCode)
	}
